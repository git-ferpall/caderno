...
if (!$user_id) {
    error_log("[SALVAR_PRODUTO] ERRO: usuário não autenticado");
...
if ($nome === '' || $tipo === '' || $atr === '') {
    error_log("[SALVAR_PRODUTO] ERRO: dados incompletos");
...
if (!$tipoVal || !$atrVal) {
    error_log("[SALVAR_PRODUTO] ERRO: valores inválidos tipo=$tipo atr=$atr");
...
if ($stmt->execute()) {
    error_log("[SALVAR_PRODUTO] SUCESSO: produto inserido id=" . $stmt->insert_id);
    echo json_encode(["ok" => true, "id" => $stmt->insert_id]);
} else {
    error_log("[SALVAR_PRODUTO] ERRO MYSQL: " . $stmt->error);
