...
try {
    ...
    // Validação
    if (!$data || !$tempo_irrigacao || empty($areas) || empty($produtos)) {
    ...
    ");
    $stmt->bind_param("isss", $propriedade_id, $data, $tempo_irrigacao, $obs);
    $stmt->execute();
    ...
    // Volume aplicado (detalhe)
    if (!empty($volume_aplicado)) {
        $campo = "volume_aplicado";
        $valor = (string)$volume_aplicado;
        $stmt->bind_param("iss", $apontamento_id, $campo, $valor);
        $stmt->execute();
    }
